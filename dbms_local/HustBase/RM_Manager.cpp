	...
	else
	{
		for(int i=2;i<pageNum;++i)
		{
			if((*(pf_bitmap+i/8) & (1<<(i%8)))!=0) //找到第一个已经分配的页
			{
				...
				for(int j=1;j<=fileHandle->rfileSubHeader->recordsPerPage;++j)   //查找一条有效的记录
				{
...
RC GetNextRec(RM_FileScan *rmFileScan,RM_Record *rec)
{
	...
	while (true)
	{
		...
		tmpRec->pData=pData+rmFileScan->pRMFileHandle->rfileSubHeader->firstRecordOffset+
			rmFileScan->pRMFileHandle->rfileSubHeader->recordSize*(tmpRec->rid->slotNum-1);
		int note=0;
		...
		if(nextSn>rmFileScan->pRMFileHandle->rfileSubHeader->recordsPerPage) //在当前页未找到有效记录
		{
			...
			if(nextPage<rmFileScan->pRMFileHandle->pf_fileHandle->pFileSubHeader->nAllocatedPages) //找到一个分配的页
			{
				...
				for(int i=1;i<=rmFileScan->pRMFileHandle->rfileSubHeader->recordsPerPage;++i)
				{
...
RC InsertRec(RM_FileHandle *fileHandle,char *pData, RID *rid)
{
	...
	for(int i=2;i<pageNum;++i)
	{
		if((*(pf_bitmap+i/8) & (1<<(i%8)))!=0) //找到一个已分配的页
		{
			...
			for(int j=1;j<fileHandle->rfileSubHeader->recordsPerPage;++j)
			{
	...
	char* offset=_pData+subHeader->firstRecordOffset+subHeader->recordSize*(rid->slotNum-1);
	memcpy(offset,pData,subHeader->recordSize);
	*(_pData+rid->slotNum/8)|=(1<<(rid->slotNum%8)); //标记记录槽有效
	subHeader->nRecords++;
	MarkDirty(pageHandle);//该页修改过则标记
	
	if(rid->slotNum>=subHeader->recordsPerPage) //如果插入记录后本页满
	{
...
RC UpdateRec (RM_FileHandle *fileHandle,const RM_Record *rec)
{
	...
	GetData(pageHandle,&pData);
	pData+=fileHandle->rfileSubHeader->firstRecordOffset+(rec->rid->slotNum-1)*fileHandle->rfileSubHeader->recordSize;
